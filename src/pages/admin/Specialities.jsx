...
import { 
  ...
  Save, Camera, ChevronDown, Check, Loader2
} from 'lucide-react';
...
const Specialities = () => {
  ...
  const humanSpecialties = ["Cardiology", "Dentist", "Neurology", "Urology", "Orthopaedics", "Dermatology", "Pediatrics", "General Physician", "Ophthalmology"];
  const vetSpecialties = ["Pet Surgery", "Canine Specialist", "Feline Specialist", "Avian Vet", "Reptile Specialist", "Equine Vet", "Pet Dermatology", "Animal Nutrition"];
  ...
  useEffect(() => { fetchSpecialists(); }, [navigate]);
  ...
  const handleSave = async (e) => {
    ...
    const isEdit = !!currentSpecialist._id;
    const url = isEdit 
      ? `http://localhost:5000/api/admin/specialists/${currentSpecialist._id}` 
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify(currentSpecialist)
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/admin/specialists/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${storedData.token}` }
      });
      if(res.ok) fetchSpecialists();
    } catch(err) {
      alert("Delete failed");
    }
  };
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
        ...
        reader.onloadend = () => setCurrentSpecialist({ ...currentSpecialist, img: reader.result });
  ...
  const categories = [
    ...
    { name: "General", icon: Stethoscope, count: getCount('Dentist') + getCount('Urology') + getCount('Physician'), type: "mixed" },
  ];
  ...
  if (isLoading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50">
        ...
        <p className="mt-4 font-bold text-slate-400 uppercase tracking-widest text-xs">Loading Tracker...</p>
    </div>
  );
          ...
          <div className="bg-white rounded-[20px] border border-slate-200 shadow-sm overflow-hidden p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredSpecialists.map((doc) => (
                    <div key={doc._id} className="border border-slate-200 rounded-2xl p-5 relative hover:shadow-lg transition-all bg-slate-50/50 group">
                        ...
                        <div className="absolute top-4 right-14 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                            <button onClick={() => handleEdit(doc)} className="p-1.5 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-blue-600 hover:border-blue-200 transition-colors shadow-sm"><Edit3 size={14} /></button>
                            <button onClick={() => handleDelete(doc._id)} className="p-1.5 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-red-600 hover:border-red-200 transition-colors shadow-sm"><Trash2 size={14} /></button>
                        </div>
                    </div>
                ))}
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-[#192a56] p-4 flex justify-between items-center">
              <h3 className="text-white font-bold text-lg">{currentSpecialist._id ? 'Update Status' : 'Add Specialist'}</h3>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="relative" ref={dropdownRef}>
                        <input type="text" placeholder="Speciality (e.g. Surgery)" value={currentSpecialist.speciality} onClick={() => setShowSpecDropdown(true)} onChange={(e) => { setCurrentSpecialist({...currentSpecialist, speciality: e.target.value}); setShowSpecDropdown(true); }} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none" />
                        ...
                        {showSpecDropdown && (
                            <ul className="absolute z-50 w-full bg-white border border-slate-200 rounded-lg shadow-xl mt-1 max-h-40 overflow-y-auto">
                                {filteredSpecs.length > 0 ? filteredSpecs.map((spec, i) => (<li key={i} onClick={() => handleSpecSelect(spec)} className="px-3 py-2 hover:bg-slate-50 cursor-pointer text-sm text-slate-700 flex justify-between items-center">{spec}{currentSpecialist.speciality === spec && <Check size={14} className="text-emerald-500" />}</li>)) : <li className="px-3 py-2 text-xs text-slate-400 italic">No match found.</li>}
                            </ul>
                        )}
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={currentSpecialist.status} onChange={(e) => setCurrentSpecialist({...currentSpecialist, status: e.target.value})} className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm focus:border-emerald-500 outline-none bg-white">
                        <option value="available">Available (Green)</option><option value="busy">In Procedure (Red)</option><option value="break">On Break (Amber)</option>
                    </select>
                  </div>
                  <div>
              ...
              <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Current Activity / Task</label><div className="relative"><Activity size={16} className="absolute top-2.5 left-3 text-slate-400" /><input type="text" required placeholder="e.g. Performing Surgery on Bruno" value={currentSpecialist.currentTask} onChange={(e) => setCurrentSpecialist({...currentSpecialist, currentTask: e.target.value})} className="w-full border border-slate-300 rounded-lg pl-9 pr-3 py-2 text-sm focus:border-emerald-500 outline-none" /></div></div>
              <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Current Location</label><div className="relative"><MapPin size={16} className="absolute top-2.5 left-3 text-slate-400" /><input type="text" required placeholder="Room No / OT" value={currentSpecialist.location} onChange={(e) => setCurrentSpecialist({...currentSpecialist, location: e.target.value})} className="w-full border border-slate-300 rounded-lg pl-9 pr-3 py-2 text-sm focus:border-emerald-500 outline-none" /></div></div>
                  <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Next Free Time</label><div className="relative"><Clock size={16} className="absolute top-2.5 left-3 text-slate-400" /><input type="text" required placeholder="e.g. 02:30 PM" value={currentSpecialist.nextFree} onChange={(e) => setCurrentSpecialist({...currentSpecialist, nextFree: e.target.value})} className="w-full border border-slate-300 rounded-lg pl-9 pr-3 py-2 text-sm focus:border-emerald-500 outline-none" /></div></div>
              </div>
              <button type="submit" className="w-full bg-[#192a56] text-white py-3 rounded-xl font-bold hover:bg-blue-900 transition-all shadow-lg flex items-center justify-center gap-2 mt-4"><Save size={18} /> {currentSpecialist._id ? 'Update Status' : 'Add to Tracker'}</button>
            </form>
          </div>
        </div>
      )}
